...
for trial = 10:13
    subplot(2,2,trial-9);
    plotFFT(data(ch,:,trial), Fs)
    title(['Channel ',num2str(ch),' trial ', num2str(trial)]);
end

ch = 26;
figure
for trial = 20:23
    subplot(2,2,trial-19);
    plotFFT(data(ch,:,trial), Fs)
    title(['Channel ',num2str(ch),' trial ', num2str(trial)]);
end
...
for i = 1:chNum
    for j = 1:triNum
        data(i,:,j) = detrend(data(i,:,j), 'constant');
    end
end
...
for trial = 10:13
    subplot(2,2,trial-9);
    plotFFT(downSampled(ch,:,trial), Fs)
    title(['DownSampled: Channel ',num2str(ch),' trial ', num2str(trial)]);
end
